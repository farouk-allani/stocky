...
export const analyzeProductImage = async (imagePath: string): Promise<AIAnalysisResult> => {
  try {
    ...
    if (USE_MOCK || !openai) {
      ...
      const mockResult = await MockAiService.analyzeProductImage(imageBuffer, filename);
...
export const generateProductDescription = async (productName: string, tags: string[]): Promise<string> => {
  try {
    // Use mock service if OpenAI is not available
    if (USE_MOCK || !openai) {
      logger.info('Using mock AI service for product description');
      return await MockAiService.generateProductDescription(productName, tags[0] || 'OTHER');
    }
    ...
    return response.choices[0]?.message?.content || `Fresh ${productName} - high quality and great value!`;
...
export const categorizeProduct = async (productName: string, tags: string[]): Promise<string> => {
  try {
    const categories = [
      'FRUITS', 'VEGETABLES', 'DAIRY', 'MEAT', 'SEAFOOD', 
      'BAKERY', 'GRAINS', 'BEVERAGES', 'SNACKS', 'FROZEN', 'OTHER'
    ];
    ...
    if (USE_MOCK || !openai) {
      ...
      if (productLower.includes('apple') || productLower.includes('fruit') || tags.includes('fruit')) {
        return 'FRUITS';
      }
      if (productLower.includes('bread') || productLower.includes('bakery') || tags.includes('bakery')) {
        return 'BAKERY';
      }
      if (productLower.includes('milk') || productLower.includes('dairy') || tags.includes('dairy')) {
        return 'DAIRY';
      }
    ...
    const response = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "user",
          content: `Categorize "${productName}" with tags ${tags.join(', ')} into one of these categories: ${categories.join(', ')}. 
