...
router.get('/my/businesses', 
  ...
  asyncHandler(async (req: any, res: any) => {
    ...
    const businessesWithStats = businesses.map(business => {
      ...
      const activeProducts = business.products.filter(p => p.status === 'ACTIVE').length;
      const discountedProducts = business.products.filter(p => p.status === 'DISCOUNTED').length;
      const expiredProducts = business.products.filter(p => p.status === 'EXPIRED').length;
