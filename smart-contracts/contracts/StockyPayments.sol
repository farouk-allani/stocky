...
contract StockyPayments {
    ...
    enum PaymentStatus { PENDING, ESCROWED, COMPLETED, REFUNDED, DISPUTED }
    ...
    // Events
    event PaymentCreated(string indexed paymentId, string orderId, uint256 amount);
    ...
    event EscrowReleased(string indexed paymentId, string sellerId, uint256 amount);
    ...
    modifier paymentExists(string memory paymentId) {
        require(bytes(payments[paymentId].paymentId).length > 0, "Payment does not exist");
        _;
    }
     ...
     */
    function escrowPayment(string memory paymentId) public onlyAuthorized paymentExists(paymentId) {
     ...
     */
    function completePayment(string memory paymentId) public onlyAuthorized paymentExists(paymentId) {
        Payment storage payment = payments[paymentId];
        require(
            payment.status == PaymentStatus.ESCROWED || 
        ...
        if (payment.status == PaymentStatus.ESCROWED) {
            require(escrowBalances[payment.sellerId] >= payment.amount, "Insufficient escrow balance");
            escrowBalances[payment.sellerId] -= payment.amount;
        }
     ...
     */
    function refundPayment(string memory paymentId) public onlyAuthorized paymentExists(paymentId) {
        Payment storage payment = payments[paymentId];
        require(
            payment.status == PaymentStatus.PENDING || 
            payment.status == PaymentStatus.ESCROWED,
     ...
     * @dev Dispute payment
     */
    function disputePayment(string memory paymentId) public onlyAuthorized paymentExists(paymentId) {
        Payment storage payment = payments[paymentId];
        require(
            payment.status == PaymentStatus.ESCROWED,
     ...
     */
    function autoReleaseEscrow(string memory paymentId) public paymentExists(paymentId) {
        Payment storage payment = payments[paymentId];
        require(payment.status == PaymentStatus.ESCROWED, "Payment not in escrow");
        require(block.timestamp >= payment.escrowReleaseTime, "Escrow period not yet ended");
     ...
     */
    function getEscrowBalance(string memory sellerId) public view returns (uint256) {
